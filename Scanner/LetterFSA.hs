...
stringFSA (source, lexeme, colNum, lineNum)
  ...
  | stringHead == Nothing                                                           = (source, lexeme, IdentifiersAndLiterals MP_STRING_LIT, colNum, lineNum)
  | otherwise                                                             = stringFSA(tail source, lexeme ++ (charToString (fromJust stringHead)), colNum+1, lineNum)
  where stringHead = if source == [] then Nothing else Just (head source)
        stringNext
              | stringHead /= Nothing && tail source /= [] = Just (source !! 1)
              | otherwise = Nothing
