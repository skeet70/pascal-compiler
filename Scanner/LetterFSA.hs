...
identifierFSA (source, lexeme, colNum, lineNum)
  | null source                                                                          = (source, lexeme, IdentifierOrLiteral MP_IDENTIFIER, colNum, lineNum)
  | isAlpha (head source)                                                                = identifierFSA(tail source, lexeme ++ [head source], colNum+1, lineNum)
  | isAlphaNum (head source) && not(null lexeme)                                         = identifierFSA(tail source, lexeme ++ [head source], colNum+1, lineNum)
  | (head source) == '_' && (tail source) /= [] && isAlphaNum (head (tail source))       = identifierFSA(tail source, lexeme ++ [head source], colNum+1, lineNum)
  | (head source) == '_' && (tail source) /= [] && not (isAlphaNum (head (tail source))) = (tail source, lexeme ++ [head source], IdentifierOrLiteral MP_IDENTIFIER, colNum, lineNum)
  | otherwise                                                                            = (source, lexeme, IdentifierOrLiteral MP_IDENTIFIER, colNum, lineNum)
...
stringFSA (source, lexeme, colNum, lineNum)
  | null source                                                                                 = (source, lexeme, ErrorCode MP_RUN_STRING, colNum, lineNum)
  | (head source) == '\'' && (tail source) /= [] && (head (tail source)) == '\'' && null lexeme = (tail (tail source), lexeme, IdentifierOrLiteral MP_STRING_LIT, colNum+2, lineNum)
  ...
  | (head source) == '\''                                                                       = (tail source, lexeme, IdentifierOrLiteral MP_STRING_LIT, colNum+1, lineNum)
  ...
  | otherwise                                                                                   = (tail source, lexeme, ErrorCode MP_ERROR, colNum, lineNum)
...
gthanFSA (source, lexeme, colNum, lineNum) 
  | (head source) == '>' && (head (tail source)) == '=' = (tail (tail source), lexeme ++ [head source] ++ [head (tail source)], Symbol MP_GEQUAL, colNum+2, lineNum)
  | (head source) == '>'                                = (tail source, lexeme ++ [head source], Symbol MP_GTHAN, colNum+1, lineNum)
  | otherwise                                           = (tail source, lexeme, ErrorCode MP_ERROR, colNum, lineNum)
...
lthanFSA (source, lexeme, colNum, lineNum)
  | (head source) == '<' && (head (tail source)) == '=' = (tail (tail source), lexeme ++ [head source] ++ [head (tail source)], Symbol MP_LEQUAL, colNum+2, lineNum)
  | (head source) == '<' && (head (tail source)) == '>' = (tail (tail source), lexeme ++ [head source] ++ [head (tail source)], Symbol MP_NEQUAL, colNum+2, lineNum)
  | (head source) == '<'                                = (tail source, lexeme ++ [head source], Symbol MP_LTHAN, colNum+1, lineNum)
  | otherwise                                           = (tail source, lexeme, ErrorCode MP_ERROR, colNum, lineNum)
...
colonFSA (source, lexeme, colNum, lineNum)
  | (head source) == ':' && (head (tail source)) == '=' = (tail (tail source), lexeme ++ [head source] ++ [head (tail source)], Symbol MP_ASSIGN, colNum+2, lineNum)
  | (head source) == ':'                                = (tail source, lexeme ++ [head source], Symbol MP_COLON, colNum+1, lineNum)
  | otherwise                                           = (tail source, lexeme, ErrorCode MP_ERROR, colNum, lineNum)